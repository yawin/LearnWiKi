...
#### Scenario: 关联即建 review_schedule
- **WHEN** 任意路径(手动/自动)创建一条 `goal_wiki_links` 记录
- **THEN** 系统在同一调用内确保该 wiki_page 存在 `review_schedules` 记录(若不存在则立即创建,默认 ease_factor = 2.5、interval = 0、mastery = 0)
