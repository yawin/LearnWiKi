...
pub fn run() {
    let db = match storage::database::Database::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
    ...
    tauri::Builder::default()
        ...
        .manage(commands::sandbox::SandboxState {
            sessions: std::sync::Mutex::new(std::collections::HashMap::new()),
        })
        .setup(move |app| {
            eprintln!("[learnwiki] App setup started");
            ...
            if let Ok(resource_dir) = app.path().resource_dir() {
                let ocr_bin = resource_dir.join("learnwiki_ocr_bin");
            ...
            eprintln!("[learnwiki] Starting capture detector...");
            detector.start(app.handle().clone());
            eprintln!("[learnwiki] Capture detector started!");
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Phase 7: Knowledge Discovery (E-7-6)
            commands::wiki::update_wiki_page_monitor,
            ...
            commands::goal::link_wiki_to_goal,
            commands::goal::unlink_wiki_from_goal,
            commands::goal::get_goal_wiki_pages,
            commands::goal::mark_goal_links_seen,
            commands::goal::get_wiki_read_status,
            commands::goal::set_wiki_read_status,
            commands::goal::get_goal_review_logs,
            commands::goal::get_goal_review_sessions,
            commands::goal::get_review_log,
            // Exam System (Phase 2 implementation pending)
            commands::review::create_exam,
            commands::review::get_exam,
            commands::review::submit_exam_answer,
            commands::review::complete_exam,
            commands::review::get_exam_history,
            commands::review::get_wiki_exam_history,
            commands::review::get_goal_exams,
            commands::review::check_mastery_flags,
            // Goal Recommendations & Auto-Matching
            commands::goal::search_goal_resources,
            commands::goal::get_goal_recommendations,
            commands::goal::dismiss_goal_recommendation,
            commands::goal::match_wiki_to_goals,
            // Old learning system commands removed (Phase 5 cleanup).
            // Functions preserved in learning.rs until DB migration.
            // Review System (Sprint 4, E-4-1)
            commands::review::create_review_schedule,
            commands::review::get_due_reviews,
            commands::review::get_due_review_for_page,
            commands::review::submit_review_feedback,
            commands::review::get_review_stats,
            commands::review::auto_create_review_schedule,
            // Knowledge Health Panel (E-4-3)
            commands::review::get_health_schedules,
            commands::review::get_review_health_stats,
            // Wiki Learning Trail (E-4-4)
            commands::review::get_wiki_learning_trail,
            // Learning Mode
            commands::review::get_learning_content,
            commands::review::mark_as_learned,
            commands::review::generate_instant_quiz,
            // Sprint 5A: Quiz & Ordering
            commands::review::generate_quiz_questions,
            commands::review::generate_ordering_steps,
            commands::review::get_available_formats,
            // Sprint 5B: Error Hunt & Cloze
            commands::review::generate_error_hunt,
            commands::review::generate_cloze,
            // Sprint 5B: Sandbox
            commands::sandbox::sandbox_start_shell,
            commands::sandbox::sandbox_write_stdin,
            commands::sandbox::sandbox_read_stdout,
            commands::sandbox::sandbox_kill_shell,
            // Sprint 6A: Explain (E-6-1)
            commands::review::generate_explain_review,
            commands::review::submit_explain_answer,
            // Sprint 6A: Variant (E-6-3)
            commands::review::generate_variant_question,
            // Sprint 6B: Comprehensive Quiz (E-6-2)
            commands::review::comprehensive_generate_quiz,
            commands::review::comprehensive_submit_quiz,
            // Sprint 6B: Adaptive Recommendations (E-6-4)
            commands::review::adaptive_get_recommendations,
            // Phase 7: Knowledge Discovery (E-7-1)
            commands::discovery::create_pending_content,
            commands::discovery::get_pending_content,
            commands::discovery::update_pending_status,
            commands::discovery::create_monitor_source,
            commands::discovery::update_monitor_source,
            commands::discovery::get_monitor_sources_for_page,
            // Phase 7: Knowledge Discovery (E-7-2 / E-7-7)
            commands::discovery::run_discovery_for_page,
            commands::discovery::run_discovery_all,
            commands::discovery::set_serpapi_key,
            commands::discovery::get_discovery_status,
            commands::discovery::record_dismissal_feedback,
            commands::discovery::get_ignored_sources,
            commands::discovery::unignore_discovery_source,
            // Phase 7: Knowledge Discovery (E-7-5: ArXiv/RSS)
            commands::discovery::search_arxiv,
            commands::discovery::test_rss_feed,
            // Phase 8: Ranking System
            commands::ranking::get_knowledge_ranking,
            commands::ranking::get_learning_ranking,
            // Phase 4: Folder Sync
...
fn setup_tray(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let show = MenuItem::with_id(app, "show", "Open LearnWiki", true, None::<&str>)?;
